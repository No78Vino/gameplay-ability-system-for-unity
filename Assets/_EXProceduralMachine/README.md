...
- **`SecondOrderDynamicsComponent`**（挂到需要次级运动的物体）→ 添加实例：
  ...
  - **影响属性**：`Position` / `Rotation`（欧拉角，自动回绕规范化）/
    `QuaternionRotation`（四元数对数空间，避免万向锁）/
    `Scale` / `Custom`（`customInput`/`customOutput` 委托）；可逐轴开关（x/y/z）。
- **组件级选项**：`timeMode`（Scaled / Unscaled / Fixed）、`resetOnEnable`（唤醒重置）、
  `initializeFromCurrentTransform`（启动用自身值初始化，防首帧瞬移）、`gizmosOn`（运行时调试线）。
- **参数含义**：
...
- **调参速查**：
  | 效果 | 频率 | 阻尼 | 缩放 |
  |---|---|---|---|
  | 相机/镜头跟随 | 1.5~2.5 | 0.8~0.9 | 0~0.3 |
  | 物理摆动（尾巴/挂件） | 0.8~1.2 | 0.3~0.5 | 1.5~2.0 |
  | UI 弹性 | 3~4 | 0.95 | 0.1 |
  | 惯性漂移（无人机位置） | 2.2 | 0.85 | 0.35 |
  | 姿态回弹（无人机倾角） | 1.4 | 0.35 | 0.55 |
- **调试**：Inspector 勾选「绘制值变化示例曲线」预览阶跃响应；Scene 勾选 `gizmosOn`
  实时查看目标点/输出连线。
...
菜单 **Tools → EXMach → 程序化动画工具箱** 打开 `EXMachTemplateWindow`（三层体系）：

- **🧰 工具（基础系统）**：次级运动、呼吸/节奏、命中枪械、可视化辅助——点「实例化」
  即创建空物体并挂载组件（默认参数开箱即用）；
- **🥘 预制菜（模板）**：`SpiderMachine`、`DroneMachine`——实例化即用，**可选预设一键改手感**
  （如无人机有「轻盈巡航 / 重载手感 / 轻快跟手」三种预设）；
- **🧸 玩具（Gameplay 小系统）**：预留分类（震屏、受击抖动、相机跟随等规划中）。
- **筛选**：🔍 搜索框（名称/描述/组件/路径）+ 分类筛选 + 家族筛选（跟随/摆动/回弹/步态/呼吸/调试）。
- **导出清单 JSON**：底部按钮把完整清单导出为 `toolbox-manifest.json`
  （分类/家族为字符串，**网页端可直接消费**——为本地主机网页工具预留的数据源）。
- **扩展**：往 `Examples/` 放新预制体 → 点「刷新」自动收录为预制菜；新工具组件在
  `Editor/Toolbox/ToolboxCatalog.cs` 的 `BuildTools()` 中添加定义。
