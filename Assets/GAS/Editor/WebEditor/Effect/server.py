...
def read_effects():
    ...
    while ws.cell(row=row, column=2).value is not None:
        ...
        if stacking_col:
            ...
            if stacking_code is not None and int(stacking_code) > 0:
                try:
                    effect["stacking"] = {
...
def write_effects(effects):
    ...
    for i, effect in enumerate(sorted(effects, key=lambda x: x["id"])):
        ...
        if modifiers_col and "Modifiers" in comp:
            ...
            mod_strs = []
            for m in mods:
                mod_strs.append(f"{m.get('attrSet', 0)};{m.get('attr', 0)};{m.get('magnitude', 0)};{ENUM_OPERATION.get(m.get('operation', 'Add'), 0)};{m.get('mmc', 0)}")
        ...
        if ability_col and "GrantedAbility" in comp:
            ...
            for a in abilities:
                ...
                remove_idx = ENUM_ABILITY_REMOVE.index(a.get("removePolicy", "None")) if a.get("removePolicy") in ENUM_ABILITY_REMOVE else 0
                ab_strs.append(f"{a.get('abilityId', 0)};{a.get('level', 1)};{act_idx};{deact_idx};{remove_idx}")
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        elif p == "/api/dropdowns":
            try:
                self.send_json({
                    "ok": True,
                    "tags": read_tags_for_dropdown(),
                    "attrSets": read_attrsets_for_dropdown(),
                    "abilities": read_abilities_for_dropdown(),
                    "cues": read_cues_for_dropdown(),
                    "effects": read_effects_for_dropdown()
                })
            except Exception as e:
                self.send_json({"ok": False, "error": str(e)}, 500)
